...
-- quality_core = mean(output_adherence, input_faithfulness) — the cross-pipeline
-- comparable quality; sheet_fit is kept as a column but excluded from quality_core
-- because for templates it measures "right template for this case", not system quality.
...
SELECT
  ...
  q.edit_burden,
  CASE WHEN q.output_adherence IS NOT NULL AND q.input_faithfulness IS NOT NULL
       THEN round((q.output_adherence + q.input_faithfulness) / 2.0, 2) END AS quality_core,
  q.judge_model,
  q.rubric_version,
...
LEFT JOIN report_quality_scores q ON q.report_id = r.id AND q.rubric_version = 'v1'
